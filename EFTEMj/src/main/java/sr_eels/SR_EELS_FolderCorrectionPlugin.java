...
public class SR_EELS_FolderCorrectionPlugin implements PlugIn {
	...
	public void run(final String arg) {
		final YesNoCancelDialog dialog = new YesNoCancelDialog(IJ.getInstance(),
			"Overwrite...", "Do you want to overwrite existing results?");
		if (dialog.cancelPressed()) return;
		if (dialog.yesPressed() == true) overwrite = true;
	...
	private void processDataSet(final String paramPath) {
		String path;
		if (paramPath.endsWith(File.separator)) path = paramPath;
		else path = paramPath + File.separator;
		final File folder = new File(path);
		final String[] list = folder.list();
		...
		for (final String item : list) {
			...
			if (subFile.isDirectory()) {
				...
				if (m.find()) {
					String tempPath;
					if (item.endsWith(File.separator)) tempPath = path + item;
					else tempPath = path + item + File.separator;
		...
		if (overwrite | !new File(path + projectionFileName).exists()) {
			...
			stack.close();
			final ImagePlus tempImage = IJ.getImage();
		...
		for (int i = 0; i < dataSets.size(); i++) {
			...
			final String resultFileName = "result_" + params.substring(0, params
				.length() - 1) + ".tif";
			if (overwrite | !new File(path + resultFileName).exists()) {
				final ImagePlus projection = IJ.openImage(path + projectionFileName);
				final ImagePlus result = correction.correctImage(projection, dataSets
					.get(i) + "Borders.txt", dataSets.get(i) + "Width.txt");
